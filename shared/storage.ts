export function sanitizeStorageFileName(fileName: string) {
  ...
  const base = original.slice(0, original.length - (extensionMatch?.[1].length ?? 0));
  const ascii = base.normalize("NFKD").replace(/[\u0300-\u036f]/g, "").replace(/[^A-Za-z0-9_-]+/g, "-").replace(/-{2,}/g, "-").replace(/^-+|-+$/g, "").slice(0, 120);
