...
const raw = detail?.artifacts.find(artifact => artifact.artifactType === "raw_video");
...
const payload = { job_id: sourceVideoId, source_video_id: sourceVideoId, owner_id: ownerId, source_url: sourceUrl, callback_url: "http://localhost:3000/api/pipeline/callback", idempotency_key: idempotencyKey };
const result = await enqueueJob({ queue: "cpu", idempotencyKey, payload });
console.log(JSON.stringify({ sourceVideoId, queue: result, artifactKey: raw.storageKey }, null, 2));
await redis.quit();
