...
export async function pipelineCallback(req: Request, res: Response) {
  try {
    ...
    if (payload.jobType === "ingest" && payload.status === "succeeded") {
      const normalized = payload.normalizedBase64 ? await storagePut(`owners/${payload.ownerId}/sources/${payload.sourceVideoId}/normalized.mp4`, Buffer.from(payload.normalizedBase64, "base64"), "video/mp4") : null;
      const audio = payload.audioBase64 ? await storagePut(`owners/${payload.ownerId}/sources/${payload.sourceVideoId}/audio.wav`, Buffer.from(payload.audioBase64, "base64"), "audio/wav") : null;
      const ingest = await completeIngestCallback({ ...payload, normalized: normalized ? { storageKey: normalized.key, mimeType: "video/mp4", byteSize: payload.normalizedBytes ?? 0 } : undefined, audio: audio ? { storageKey: audio.key, mimeType: "audio/wav", byteSize: payload.audioBytes ?? 0 } : undefined });
