...
export const appRouter = router({
  ...
  dashboard: router({
    overview: protectedProcedure.query(({ ctx }) =>
      getPipelineOverview(ctx.user.id)
    ),
  ...
  videos: router({
    ...
    register: protectedProcedure
      ...
      .mutation(({ ctx, input }) =>
        createSourceVideo({ ...input, ownerId: ctx.user.id })
      ),
    upload: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        const source = await createSourceVideo({
          ownerId: ctx.user.id,
          title: input.title,
          sourceType: "upload",
        ...
        if (content.byteLength > 6 * 1024 * 1024)
          throw new Error("Arquivo acima do limite da primeira versão");
        const safeFileName = sanitizeStorageFileName(input.fileName);
        const stored = await storagePut(
          `owners/${ctx.user.id}/sources/${source.id}/${safeFileName}`,
          content,
          input.mimeType
        );
        ...
        const queue = await enqueueJob({
          queue: "cpu",
          idempotencyKey: `ingest:${source.id}:${input.idempotencyKey}`,
          payload: {
            job_id: source.id,
    ...
    start: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const pipelineDetail = await getPipelineDetail(
          ctx.user.id,
          result.videoId
        );
        ...
        for (const stage of result.stages) {
          const queue =
            stage === "transcribe"
              ? "gpu"
              : stage === "detect_highlights"
                ? "llm"
                : "cpu";
          const payload =
            stage === "ingest"
              ? {
                  job_id: result.videoId,
          ...
          if (stage === "ingest" && !sourceUrl)
            throw new Error(
              "Artefato bruto não encontrado para iniciar a ingestão"
            );
          await enqueueJob({
            queue,
            idempotencyKey: `${stage}:${result.videoId}`,
            payload,
          });
        }
        return result;
    ...
    cancel: protectedProcedure
      .input(z.object({ id: z.number().int().positive() }))
      .mutation(({ ctx, input }) =>
        cancelSourceVideoPipeline(ctx.user.id, input.id)
      ),
    retry: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        for (const stage of result.stages) {
          ...
          if (stage === "ingest" && !sourceUrl)
            throw new Error(
              "Artefato bruto não encontrado para reprocessar a ingestão"
            );
  ...
  review: router({
    list: protectedProcedure.query(({ ctx }) =>
      listReviewCandidates(ctx.user.id)
    ),
    update: protectedProcedure
      .input(
        z.object({
          ...
          suggestedTitle: z.string().max(255).optional(),
        })
      )
      .mutation(({ ctx, input }) =>
  ...
  pipeline: router({
    reportEvent: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        const severity =
          input.event === "pipeline_failed" ||
          input.event === "publication_failed"
            ? "critical"
            : input.event === "score_anomaly"
              ? "warning"
              : "info";
        const alert = await createPipelineAlert({
          ownerId: ctx.user.id,
  ...
  integrations: router({
    list: protectedProcedure.query(({ ctx }) =>
      listIntegrationSettings(ctx.user.id)
    ),
    save: protectedProcedure
      .input(
        z.object({
          ...
          publishEndpoint: z.string().url().optional().or(z.literal("")),
      ...
      .mutation(({ ctx, input }) =>
        upsertIntegrationSetting({
          ...input,
          publishEndpoint: input.publishEndpoint || undefined,
          ownerId: ctx.user.id,
        })
      ),
  }),
  alerts: router({
    ...
    markRead: protectedProcedure
      .input(z.object({ id: z.number().int().positive() }))
      .mutation(({ ctx, input }) => markAlertRead(ctx.user.id, input.id)),
  }),
  analytics: router({
  ...
  ai: router({
    generateMetadata: protectedProcedure
      ...
      .mutation(({ input }) =>
        generateClipMetadata(input.transcript, input.category)
      ),
    detectHighlightsPreview: protectedProcedure
      .input(
        z.object({
          transcriptChunk: z.string().min(20).max(30000),
          ...
          messages: [
            {
              role: "system",
              content:
                "Você é editor especialista em cortes virais. Retorne apenas candidatos autocontidos, sem inventar contexto.",
            },
            {
              role: "user",
              content: `Analise esta transcrição em ${input.language}. Encontre trechos de 20 a 90 segundos com gancho, arco narrativo e reação emocional.\n\n${input.transcriptChunk}`,
            },
          ],
          response_format: {
            type: "json_schema",
