...
describe("pipeline contracts", () => {
  ...
  it("splits transcript windows with overlap", () => {
    const segments = Array.from({ length: 4 }, (_, index) => ({ id: index, start: index * 600, end: index * 600 + 500, text: `segment ${index}`, words: [] }));
  ...
  it("keeps only the highest scoring overlapping candidate", () => {
    const result = removeOverlappingCandidates([{ start: 0, end: 60, finalScore: 90, category: "educativo" }, { start: 20, end: 80, finalScore: 70, category: "curioso" }, { start: 100, end: 150, finalScore: 80, category: "engraçado" }]);
    expect(result).toHaveLength(2);
    expect(result[0]?.finalScore).toBe(90);
  });

  it("builds word-level ASS karaoke events", () => {
    const ass = buildAssKaraoke([{ word: "Olá", start: 0, end: 0.5 }, { word: "mundo", start: 0.5, end: 1.2 }]);
    expect(ass).toContain("\\k50Olá");
    expect(ass).toContain("\\k70mundo");
  });

  it("creates a 9:16 render contract and ffmpeg filter", () => {
    const request = createVerticalRenderRequest({ sourceArtifactKey: "input.mp4", outputArtifactKey: "vertical.mp4", startSeconds: 10, endSeconds: 50, cropMode: "center" });
  ...
  it("normalizes generated metadata to platform-safe limits", () => {
    const result = validateGeneratedMetadata({ titles: ["  título longo ".repeat(10)], description: " descrição ".repeat(100), hashtags: ["#cortes", "#cortes", "ia"], thumbnailText: "texto chamativo" });
  ...
  it("recalibrates weights only with sufficient observations", () => {
    ...
    expect(recalibrateWeights([{ predictedScore: 80, retentionRate: 80, approved: true }], current)).toEqual(current);
    const observations = Array.from({ length: 10 }, () => ({ predictedScore: 80, retentionRate: 90, approved: true }));
  ...
  it("creates idempotent scheduled publication requests", () => {
    const result = createPublishRequest({ platform: "youtube", clipId: 7, mediaUrl: "https://storage.example/clip.mp4", title: "Corte", description: "Descrição", hashtags: ["#shorts"], scheduledAt: new Date("2026-08-20T11:00:00Z"), lastScheduledAt: new Date("2026-08-20T09:00:00Z") });
  ...
  it("enforces a minimum publication gap", () => {
    ...
    expect(isPublicationAllowed(last, new Date("2026-08-20T10:30:00Z"), 60)).toBe(false);
    expect(isPublicationAllowed(last, new Date("2026-08-20T11:00:00Z"), 60)).toBe(true);
  });
});
