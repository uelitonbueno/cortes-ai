...
const labels: Record<string, string> = {
  ...
  detecting: "Detectando highlights",
...
export default function Videos() {
  ...
  const register = trpc.videos.register.useMutation({
    onSuccess: () => {
      toast.success("Vídeo registrado no pipeline");
  ...
  const upload = trpc.videos.upload.useMutation({
    onSuccess: () => {
      toast.success("Vídeo enviado ao storage seguro");
      utils.videos.list.invalidate();
      utils.dashboard.overview.invalidate();
      setTitle("");
      setFile(null);
        ...
        <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
          <div>
            ...
            <p className="mt-2 text-slate-500">
              Registre vídeos longos e acompanhe o processamento de ponta a
              ponta.
            </p>
          </div>
        </div>
        <Card className="border-0 bg-[#102a43] text-white shadow-sm shadow-slate-200/70">
          <CardContent className="grid gap-5 p-6 md:grid-cols-[1fr_auto] md:items-end">
            ...
            >
              ...
              {file ? "Enviar vídeo" : "Registrar vídeo"}
            </Button>
          </CardContent>
        </Card>
        <Card className="border-0 bg-white shadow-sm shadow-slate-200/70">
          <CardHeader>
            <CardTitle className="text-lg">Histórico de ingestão</CardTitle>
          </CardHeader>
          <CardContent>
            {videos.isLoading ? (
                ...
                {videos.data.map(video => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="mt-1 text-xs text-slate-500">
                          {video.sourceType} · versão {video.processingVersion}
                        </p>
                        <Progress
                          value={progress[video.status] ?? 0}
                          className="mt-2 h-1.5 w-36"
                        />
                        {video.errorMessage && (
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      >
                        {labels[video.status] ?? video.status}
                      </Badge>
                      <Link
                        href={`/videos/${video.id}`}
                        className="rounded-lg p-2 text-cyan-700 hover:bg-cyan-50"
                        aria-label={`Abrir detalhe de ${video.title}`}
                      >
              ...
              <div className="rounded-2xl border border-dashed border-slate-200 p-12 text-center">
                ...
                <p className="mt-1 text-sm text-slate-500">
                  A primeira versão registra a fonte e prepara o contrato para
                  upload binário e workers.
                </p>
              </div>
            )}
          </CardContent>
